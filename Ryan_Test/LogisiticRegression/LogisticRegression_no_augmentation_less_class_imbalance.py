...
import BirdClef_dataset_loader_no_augmentation as BirdClef_dataset_loader
import torchaudio
import numpy as np
...
def get_data(is_3d=True):
    ...
    train_dataset = BirdClef_dataset_loader.BirdClefDataset(train_df, mel_spectrogram, sr, duration, True, is_3d)
    valid_dataset = BirdClef_dataset_loader.BirdClefDataset(valid_df, mel_spectrogram, sr, duration, False, is_3d)
    test_dataset = BirdClef_dataset_loader.BirdClefDataset(test_df, mel_spectrogram, sr, duration, False, is_3d)
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        for i, (x, y) in enumerate(valid_loop):
            ...
            valid_loop.set_postfix_str(f"Accuracy: {round(accuracy, 4)} and valid loss: {round(valid_loss / (i + 1), 3)} and f1: {round(f1, 3)}")
        ...
        if (valid_accuracy > best_accuracy):
            print(f"Accuracy improved from {best_accuracy} -> {valid_accuracy}. Saving Model")
        ...
        if early_stop < 0:
            print("Early stopping, no increase in accuracy over the past 10 epochs")
            break
    ...
    plt.suptitle(f"Logistic Regression No Augmentation Less Class Imbalance")
    ...
    plt.savefig(f'./logistic_regression_no_augmentation_class_imbalance.jpg')
    ...
    np.savetxt('train_epoch_losses_logistic_regression_no_augmentation_class_imbalance.txt', train_losses, delimiter=',')
    np.savetxt('valid_epoch_losses_logistic_regression_no_augmentation_class_imbalance.txt', valid_losses, delimiter=',')
    np.savetxt('valid_epoch_accuracy_logistic_regression_no_augmentation_class_imbalance.txt', valid_accuracies, delimiter=',')
    np.savetxt('valid_epoch_f1_logistic_regression_no_augmentation_class_imbalance.txt', valid_epoch_f1, delimiter=',')
    np.savetxt('test_loss_accuracy_f1_logistic_regression_no_augmentation_class_imbalance.txt', np.array([test_loss, test_accuracy, test_f1]))
