...
def run(fold):
  ...
  for epoch in range(epochs):
    ...
    if (last_accuracy < valid_accuracy):
        ...
        torch.save(resnet152.state_dict(), f'./resnet152.bin')
        early_stop = 5
    else:
      early_stop -= 1
...
if __name__ == "__main__":
  ...
  for i in range(fold):
    ...
    np.savetxt('train_epoch_losses_resnet152.txt', train_epoch_losses, delimiter=',')
    np.savetxt('valid_epoch_losses_resnet152.txt', valid_epoch_losses, delimiter=',')
    np.savetxt('valid_epoch_accuracy_resnet152.txt', valid_epoch_accuracy, delimiter=',')
    np.savetxt('valid_epoch_f1_resnet152.txt', valid_epoch_f1, delimiter=',')
    np.savetxt('test_loss_accuracy_f1_resnet152.txt', np.array([test_loss, test_accuracy, test_f1]))