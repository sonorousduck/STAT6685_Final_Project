...
def get_data(fold):
    ...
    train_dataset = BirdClefDataset(train_df, mel_spectrogram, sr, duration)
    valid_dataset = BirdClefDataset(valid_df, mel_spectrogram, sr, duration)
...
def run(fold):
  ...
  torch.save(resnet50.state_dict(), f'./resnet50_unfrozen_deeper_output.bin')
...
for i in range(fold):
  ...
  plt.suptitle(f"All Unfrozen, Deeper output, Fold 4")
  ...
  plt.savefig(f'./loss_accuracy_unfrozen_deeper_output_fold_4.jpg')