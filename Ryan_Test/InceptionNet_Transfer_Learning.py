...
def run(fold):
  ...
  optimizer = Adam(inception.parameters(), lr=1e-4)
  epochs = 1000
  train_epoch_losses = []
  valid_epoch_losses = []
  ...
  if load:
    inception.load_state_dict(torch.load('./inception.bin'))
  ...
  for epoch in range(epochs):
    ...
    for i, (x, y) in enumerate(loop):
      ...
      outputs, aux_outputs = inception(x)
    ...
    if early_stop <= 0:
      if valid_epoch_accuracy[-1] < valid_epoch_accuracy[-10]:
        print(f"Early stopping because most recent accuracy {valid_epoch_accuracy[-1]}, 10 ago was {valid_epoch_accuracy[-10]}")
        break
      else:
        early_stop = 10

  torch.save(inception.state_dict(), f'./inception.bin')
...
#   )
  ...
  inception.AuxLogits.fc = nn.Linear(in_features=768, out_features=152)
  inception.fc = nn.Linear(in_features=2048, out_features=152)
  inception.to(device)

  train_epoch_losses, valid_epoch_losses, valid_epoch_accuracy = run(4)

  fig, ax = plt.subplots(1, 2, figsize=(10, 7))
  plt.suptitle(f"Inception Net")
  ...
  plt.savefig(f'./inception_net.jpg')