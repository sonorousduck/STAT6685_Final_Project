...
def run(fold):
  ...
  if load:
    vgg19.load_state_dict(torch.load('./vgg19_no_augmentation_less_class_imbalance.bin'))
  ...
  for epoch in range(epochs):
    ...
    if last_accuracy < valid_accuracy:
      ...
      torch.save(vgg19.state_dict(), f'./vgg19_no_augmentation_less_class_imbalance.bin')
      early_stop = 10
    else:
      early_stop -= 1
...
for i in range(fold):
  ...
  plt.savefig(f'./VGG19_no_augmentation_class_imbalance.jpg')
