...
def run(fold):
  ...
  optimizer = Adam(vgg19.parameters(), lr=1e-4)
  ...
  if load:
    vgg19.load_state_dict(torch.load('./vgg19_no_augmentation.bin'))
  ...
  for epoch in range(epochs):
    ...
    if last_accuracy < valid_accuracy:
      ...
      torch.save(vgg19.state_dict(), f'./vgg19_no_augmentation.bin')
      early_stop = 10
    else:
      early_stop -= 1

    if early_stop <= 0:
      print(f"Early stopping because most recent accuracy {valid_epoch_accuracy[-1]}, 5 ago was {valid_epoch_accuracy[-20]}")
      break
...
for i in range(fold):
  ...
  plt.savefig(f'./VGG19_no_augmentation.jpg')
  ...
  np.savetxt('train_epoch_losses_regnet_no_augmentation.txt', train_epoch_losses, delimiter=',')
  np.savetxt('valid_epoch_losses_regnet_no_augmentation.txt', valid_epoch_losses, delimiter=',')
  np.savetxt('valid_epoch_accuracy_regnet_no_augmentation.txt', valid_epoch_accuracy, delimiter=',')
  np.savetxt('valid_epoch_f1_regnet_no_augmentation.txt', valid_epoch_f1, delimiter=',')
  np.savetxt('test_loss_accuracy_f1_regnet_no_augmentation.txt', np.array([test_loss, test_accuracy, test_f1]))
