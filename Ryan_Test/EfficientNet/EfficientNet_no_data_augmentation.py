...
def run(fold):
  ...
  if load:
    efficient_net.load_state_dict(torch.load('./efficient_net_no_data_augmentation.bin'))
  ...
  for epoch in range(epochs):
    ...
    if (last_accuracy < valid_accuracy):
      ...
      torch.save(efficient_net.state_dict(), f'./efficient_net_no_data_augmentation.bin')
      early_stop = 10
    else:
      early_stop -= 1
...
for i in range(fold):
  ...
  plt.suptitle(f"EfficientNet No Data Augmentation")
  ...
  ax[2].legend()
  plt.savefig(f'./efficient_net_no_data_augmentation.jpg')

  np.savetxt('train_epoch_losses_efficient_net_no_augmentation.txt', train_epoch_losses, delimiter=',')
  np.savetxt('valid_epoch_losses_efficient_net_no_augmentation.txt', valid_epoch_losses, delimiter=',')
  np.savetxt('valid_epoch_accuracy_efficient_net_no_augmentation.txt', valid_epoch_accuracy, delimiter=',')
  np.savetxt('valid_epoch_f1_efficient_net_no_augmentation.txt', valid_epoch_f1, delimiter=',')
  np.savetxt('test_loss_accuracy_f1_efficient_net_no_augmentation.txt', np.array([test_loss, test_accuracy, test_f1]))