...
def run(fold):
  ...
  if load:
    efficient_net.load_state_dict(torch.load('./efficient_net_no_data_augmentation_less_class_imbalance.bin'))
  ...
  for epoch in range(epochs):
    ...
    for i, (x, y) in enumerate(loop_validation):
      ...
      loop_validation.set_postfix_str(f"Loss: {round(temp_loss / (i + 1), 3)} Accuracy: {round(accuracy, 3)} F1: {round(f1, 3)}")
...
for i in range(fold):
  ...
  plt.suptitle(f"EfficientNet No Augmentation Less Class Imbalance")
  ...
  plt.savefig(f'./efficient_net_no_data_augmentation_class_imbalance.jpg')

  np.savetxt('train_epoch_losses_efficient_net_no_augmentation_class_imbalance.txt', train_epoch_losses, delimiter=',')
  np.savetxt('valid_epoch_losses_efficient_net_no_augmentation_class_imbalance.txt', valid_epoch_losses, delimiter=',')
  np.savetxt('valid_epoch_accuracy_efficient_net_no_augmentation_class_imbalance.txt', valid_epoch_accuracy, delimiter=',')
  np.savetxt('valid_epoch_f1_efficient_net_no_augmentation_class_imbalance.txt', valid_epoch_f1, delimiter=',')
  np.savetxt('test_loss_accuracy_f1_efficient_net_no_augmentation_class_imbalance.txt', np.array([test_loss, test_accuracy, test_f1]))