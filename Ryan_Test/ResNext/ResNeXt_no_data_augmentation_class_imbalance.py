...
def run(fold):
  ...
  if load:
    resnext.load_state_dict(torch.load('./resnext_no_data_augmentation_class_imbalance.bin'))
  ...
  for epoch in range(epochs):
    ...
    if (last_accuracy < valid_accuracy):
        ...
        torch.save(resnext.state_dict(), f'./resnext_no_data_augmentation_class_imbalance.bin')
        early_stop = 5
    else:
      early_stop -= 1
...
if __name__ == "__main__":
  ...
  for i in range(fold):
    ...
    plt.suptitle(f"ResNext No Data Augmentation Less Class Imbalancing")
    ...
    plt.savefig(f'./resnext_no_data_augmentation_class_imbalance.jpg')
    ...
    np.savetxt('train_epoch_losses_resnext_no_data_augmentation_class_imbalance.txt', train_epoch_losses, delimiter=',')
    np.savetxt('valid_epoch_losses_resnext_no_data_augmentation_class_imbalance.txt', valid_epoch_losses, delimiter=',')
    np.savetxt('valid_epoch_accuracy_resnext_no_data_augmentation_class_imbalance.txt', valid_epoch_accuracy, delimiter=',')
    np.savetxt('valid_epoch_f1_resnext_no_data_augmentation_class_imbalance.txt', valid_epoch_f1, delimiter=',')
    np.savetxt('test_loss_accuracy_f1_resnext_no_data_augmentation_class_imbalance.txt', np.array([test_loss, test_accuracy, test_f1]), delimiter=',')