...
def run(fold):
  ...
  if load:
    regnet.load_state_dict(torch.load('./regnet_no_augmentation.bin'))
  ...
  for epoch in range(epochs):
    ...
    if (last_accuracy < valid_accuracy):
        ...
        torch.save(regnet.state_dict(), f'./regnet_no_augmentation.bin')
        early_stop = 10
    else:
      early_stop -= 1
...
for i in range(fold):
  ...
  plt.suptitle(f"RegNet No Augmentation")
  ...
  plt.savefig(f'./regnet_no_augmentation.jpg')
